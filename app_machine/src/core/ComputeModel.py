...
class ComputeModel:
    def __init__(
                self,df_X:pd.DataFrame,y:pd.Series,\
                 ...
                 l_order:list=None,positive_mod=None,grid_search=False,
        ...
        if not self.grid_search:
            ...
            self.model=Model(model_name,grid_search=GridSearchCV(Model(model_name).model, self.params_grid, verbose=verbose, cv=cv))
