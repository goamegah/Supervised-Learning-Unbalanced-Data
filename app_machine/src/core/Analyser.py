...
class Analyser:
    ...
    def prop_churn_by_numerical_hist(self,df_c:pd.DataFrame,outcome:str,positive_mod:str,\
                                numericals_name:str,ax:Axes,n_bins=5,
                                alpha=.75,density=True,loc="upper right") -> None:
        summary=self.summary(df_c)
        ...
        counts, bins, _ =ax.hist([churn[numericals_name],non_churn[numericals_name]],bins=n_bins,alpha=alpha,\
                label=[positive_mod,f"Not {positive_mod}"],color=["blue","black"],\
    ...
    def prop_churn_by_numerical_boxplot(self,df_c:pd.DataFrame,outcome:str,positive_mod:str, \
                                        numericals_name:str,ax:Axes):
        summary=self.summary(df_c)
        if not numericals_name in summary["features"]["quantitative_columns"] or \
                not outcome in summary["features"]["qualitative_columns"]:
            raise Exception(f"{outcome} should be a qualitative columns  and \
