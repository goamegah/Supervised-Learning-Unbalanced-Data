...
class ComputeModel:
    ...
    def __init__(
                self,df_X:pd.DataFrame,y:pd.Series,\
                 model_name:str,process:Processing,hyperparameters:dict=None,
                 l_order:list=None,positive_mod=None,
                 grid_search=False,random_search=False,stratify=True,
                 params_grid=None,cv=3,verbose=1
                 ):
        self.df_X=df_X
        self.y=y
        ...
        if not self.search:
            ...
            if random_search:
                search=RandomizedSearchCV(Model(model_name).model,param_distributions=self.params_grid,cv=cv,verbose=verbose)
            else:
    ...
    def split(self,test_size=.2,random_state=42) ->None:
        ...
        if self.l_order:
            self.dict_split={
                "arrays":{"X_train":X_train,"y_train":y_train.reshape(-1),"X_test":X_test,"y_test":y_test.reshape(-1)},
    ...
    def permutation_importance_model(self,scoring="roc_auc"):
        return self.model.permutation_importance_model(self.dict_split["arrays"]["X_train"],\
                                                       self.dict_split["arrays"]["y_train"],scoring)
